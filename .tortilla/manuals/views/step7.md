...
@@ -153,38 +119,36 @@
 ...
 ┊155┊121┊  Group: {
-┊156┊   ┊    users(group) {
-┊157┊   ┊      return group.getUsers();
-┊158┊   ┊    },
-┊159┊   ┊    messages(group, args) {
-┊160┊   ┊      return Message.findAll({
-┊161┊   ┊        where: { groupId: group.id },
-┊162┊   ┊        order: [['createdAt', 'DESC']],
-┊163┊   ┊        limit: args.limit,
-┊164┊   ┊        offset: args.offset,
-┊165┊   ┊      });
+┊   ┊122┊    users(group, args, ctx) {
+┊   ┊123┊      return groupLogic.users(group, args, ctx);
+┊   ┊124┊    },
+┊   ┊125┊    messages(group, args, ctx) {
+┊   ┊126┊      return groupLogic.messages(group, args, ctx);
 ┊166┊127┊    },
 ┊167┊128┊  },
 ┊168┊129┊  Message: {
-┊169┊   ┊    to(message) {
-┊170┊   ┊      return message.getGroup();
+┊   ┊130┊    to(message, args, ctx) {
+┊   ┊131┊      return messageLogic.to(message, args, ctx);
 ┊171┊132┊    },
-┊172┊   ┊    from(message) {
-┊173┊   ┊      return message.getUser();
+┊   ┊133┊    from(message, args, ctx) {
+┊   ┊134┊      return messageLogic.from(message, args, ctx);
 ┊174┊135┊    },
 ┊175┊136┊  },
 ┊176┊137┊  User: {
-┊177┊   ┊    messages(user) {
-┊178┊   ┊      return Message.findAll({
-┊179┊   ┊        where: { userId: user.id },
-┊180┊   ┊        order: [['createdAt', 'DESC']],
-┊181┊   ┊      });
+┊   ┊138┊    email(user, args, ctx) {
+┊   ┊139┊      return userLogic.email(user, args, ctx);
+┊   ┊140┊    },
+┊   ┊141┊    friends(user, args, ctx) {
+┊   ┊142┊      return userLogic.friends(user, args, ctx);
+┊   ┊143┊    },
+┊   ┊144┊    groups(user, args, ctx) {
+┊   ┊145┊      return userLogic.groups(user, args, ctx);
 ┊182┊146┊    },
-┊183┊   ┊    groups(user) {
-┊184┊   ┊      return user.getGroups();
+┊   ┊147┊    jwt(user, args, ctx) {
+┊   ┊148┊      return userLogic.jwt(user, args, ctx);
 ┊185┊149┊    },
-┊186┊   ┊    friends(user) {
-┊187┊   ┊      return user.getFriends();
+┊   ┊150┊    messages(user, args, ctx) {
+┊   ┊151┊      return userLogic.messages(user, args, ctx);
 ┊188┊152┊    },
 ┊189┊153┊  },
 ┊190┊154┊};
```
...
@@ -96,24 +96,28 @@
 ┊ 96┊ 96┊    messageAdded: {
 ┊ 97┊ 97┊      subscribe: withFilter(
 ┊ 98┊ 98┊        () => pubsub.asyncIterator(MESSAGE_ADDED_TOPIC),
-┊ 99┊   ┊        (payload, args) => {
-┊100┊   ┊          return Boolean(
-┊101┊   ┊            args.groupIds &&
-┊102┊   ┊            ~args.groupIds.indexOf(payload.messageAdded.groupId) &&
-┊103┊   ┊            args.userId !== payload.messageAdded.userId, // don't send to user creating message
-┊104┊   ┊          );
+┊   ┊ 99┊        (payload, args, ctx) => {
+┊   ┊100┊          return ctx.user.then((user) => {
+┊   ┊101┊            return Boolean(
+┊   ┊102┊              args.groupIds &&
+┊   ┊103┊              ~args.groupIds.indexOf(payload.messageAdded.groupId) &&
+┊   ┊104┊              user.id !== payload.messageAdded.userId, // don't send to user creating message
+┊   ┊105┊            );
+┊   ┊106┊          });
 ┊105┊107┊        },
 ┊106┊108┊      ),
 ┊107┊109┊    },
 ┊108┊110┊    groupAdded: {
 ┊109┊111┊      subscribe: withFilter(
 ┊110┊112┊        () => pubsub.asyncIterator(GROUP_ADDED_TOPIC),
-┊111┊   ┊        (payload, args) => {
-┊112┊   ┊          return Boolean(
-┊113┊   ┊            args.userId &&
-┊114┊   ┊            ~map(payload.groupAdded.users, 'id').indexOf(args.userId) &&
-┊115┊   ┊            args.userId !== payload.groupAdded.users[0].id, // don't send to user creating group
-┊116┊   ┊          );
+┊   ┊113┊        (payload, args, ctx) => {
+┊   ┊114┊          return ctx.user.then((user) => {
+┊   ┊115┊            return Boolean(
+┊   ┊116┊              args.userId &&
+┊   ┊117┊              ~map(payload.groupAdded.users, 'id').indexOf(args.userId) &&
+┊   ┊118┊              user.id !== payload.groupAdded.users[0].id, // don't send to user creating group
+┊   ┊119┊            );
+┊   ┊120┊          });
 ┊117┊121┊        },
 ┊118┊122┊      ),
...
@@ -52,6 +53,7 @@
 ┊52┊53┊
 ┊53┊54┊const AppNavigator = StackNavigator({
 ┊54┊55┊  Main: { screen: MainScreenNavigator },
+┊  ┊56┊  Signin: { screen: Signin },
 ┊55┊57┊  Messages: { screen: Messages },
 ┊56┊58┊  GroupDetails: { screen: GroupDetails },
 ┊57┊59┊  NewGroup: { screen: NewGroup },
```
```diff
@@ -148,6 +150,7 @@
 ┊148┊150┊});
 ┊149┊151┊
 ┊150┊152┊const userQuery = graphql(USER_QUERY, {
+┊   ┊153┊  skip: ownProps => true, // fake it -- we'll use ownProps with auth
 ┊151┊154┊  options: () => ({ variables: { id: 1 } }), // fake the user for now
 ┊152┊155┊  props: ({ data: { loading, user, refetch, subscribeToMore } }) => ({
 ┊153┊156┊    loading,
```
...
@@ -20,35 +21,10 @@
 ┊20┊21┊
 ┊21┊22┊import { wsClient } from './app';
 ┊22┊23┊
...
-┊46┊  ┊);
-┊47┊  ┊
 ┊48┊24┊// tabs in main screen
 ┊49┊25┊const MainScreenNavigator = TabNavigator({
 ┊50┊26┊  Chats: { screen: Groups },
-┊51┊  ┊  Settings: { screen: TestScreen('Settings') },
+┊  ┊27┊  Settings: { screen: Settings },
 ┊52┊28┊});
 ┊53┊29┊
 ┊54┊30┊const AppNavigator = StackNavigator({
```
```diff
@@ -73,6 +49,27 @@
 ┊73┊49┊export const navigationReducer = (state = initialNavState, action) => {
 ┊74┊50┊  let nextState;
 ┊75┊51┊  switch (action.type) {
+┊  ┊52┊    case REHYDRATE:
+┊  ┊53┊      // convert persisted data to Immutable and confirm rehydration
+┊  ┊54┊      if (!action.payload.auth || !action.payload.auth.jwt) {
+┊  ┊55┊        const { routes, index } = state;
+┊  ┊56┊        if (routes[index].routeName !== 'Signin') {
+┊  ┊57┊          nextState = AppNavigator.router.getStateForAction(
+┊  ┊58┊            NavigationActions.navigate({ routeName: 'Signin' }),
+┊  ┊59┊            state,
+┊  ┊60┊          );
+┊  ┊61┊        }
+┊  ┊62┊      }
+┊  ┊63┊      break;
+┊  ┊64┊    case 'LOGOUT':
+┊  ┊65┊      const { routes, index } = state;
+┊  ┊66┊      if (routes[index].routeName !== 'Signin') {
+┊  ┊67┊        nextState = AppNavigator.router.getStateForAction(
+┊  ┊68┊          NavigationActions.navigate({ routeName: 'Signin' }),
+┊  ┊69┊          state,
+┊  ┊70┊        );
+┊  ┊71┊      }
+┊  ┊72┊      break;
 ┊76┊73┊    default:
 ┊77┊74┊      nextState = AppNavigator.router.getStateForAction(action, state);
 ┊78┊75┊      break;
```
...
-┊146┊   ┊  nav: state.nav,
+┊   ┊145┊const mapStateToProps = ({ auth, nav }) => ({
+┊   ┊146┊  auth,
+┊   ┊147┊  nav,
 ┊147┊148┊});
 ┊148┊149┊
 ┊149┊150┊const userQuery = graphql(USER_QUERY, {
-┊150┊   ┊  skip: ownProps => true, // fake it -- we'll use ownProps with auth
-┊151┊   ┊  options: () => ({ variables: { id: 1 } }), // fake the user for now
+┊   ┊151┊  skip: ownProps => !ownProps.auth || !ownProps.auth.jwt,
+┊   ┊152┊  options: ownProps => ({ variables: { id: ownProps.auth.id } }),
 ┊152┊153┊  props: ({ data: { loading, user, refetch, subscribeToMore } }) => ({
 ┊153┊154┊    loading,
 ┊154┊155┊    user,
```
...
+┊  ┊18┊import { connect } from 'react-redux';
 ┊18┊19┊
 ┊19┊20┊import { wsClient } from '../app';
 ┊20┊21┊import Message from '../components/message.component';
```
```diff
@@ -159,7 +160,6 @@
 ┊159┊160┊  send(text) {
 ┊160┊161┊    this.props.createMessage({
 ┊161┊162┊      groupId: this.props.navigation.state.params.groupId,
-┊162┊   ┊      userId: 1, // faking the user for now
 ┊163┊163┊      text,
 ┊164┊164┊    }).then(() => {
 ┊165┊165┊      this.flatList.scrollToIndex({ index: 0, animated: true });
```
```diff
@@ -171,7 +171,7 @@
 ┊171┊171┊  renderItem = ({ item: message }) => (
 ┊172┊172┊    <Message
 ┊173┊173┊      color={this.state.usernameColors[message.from.username]}
-┊174┊   ┊      isCurrentUser={message.from.id === 1} // for now until we implement auth
+┊   ┊174┊      isCurrentUser={message.from.id === this.props.auth.id}
...
@@ -212,6 +212,10 @@
 ┊212┊212┊}
 ┊213┊213┊
 ┊214┊214┊Messages.propTypes = {
+┊   ┊215┊  auth: PropTypes.shape({
+┊   ┊216┊    id: PropTypes.number,
+┊   ┊217┊    username: PropTypes.string,
+┊   ┊218┊  }),
 ┊215┊219┊  createMessage: PropTypes.func,
 ┊216┊220┊  navigation: PropTypes.shape({
 ┊217┊221┊    navigate: PropTypes.func,
```
```diff
@@ -270,10 +274,10 @@
 ┊270┊274┊});
 ┊271┊275┊
 ┊272┊276┊const createMessageMutation = graphql(CREATE_MESSAGE_MUTATION, {
-┊273┊   ┊  props: ({ mutate }) => ({
-┊274┊   ┊    createMessage: ({ text, userId, groupId }) =>
+┊   ┊277┊  props: ({ ownProps, mutate }) => ({
+┊   ┊278┊    createMessage: ({ text, groupId }) =>
 ┊275┊279┊      mutate({
-┊276┊   ┊        variables: { text, userId, groupId },
+┊   ┊280┊        variables: { text, groupId },
 ┊277┊281┊        optimisticResponse: {
 ┊278┊282┊          __typename: 'Mutation',
 ┊279┊283┊          createMessage: {
```
```diff
@@ -283,8 +287,8 @@
 ┊283┊287┊            createdAt: new Date().toISOString(), // the time is now!
 ┊284┊288┊            from: {
 ┊285┊289┊              __typename: 'User',
-┊286┊   ┊              id: 1, // still faking the user
-┊287┊   ┊              username: 'Justyn.Kautzer', // still faking the user
+┊   ┊290┊              id: ownProps.auth.id,
+┊   ┊291┊              username: ownProps.auth.username,
 ┊288┊292┊            },
 ┊289┊293┊            to: {
 ┊290┊294┊              __typename: 'Group',
```
```diff
@@ -320,7 +324,7 @@
 ┊320┊324┊          const userData = store.readQuery({
 ┊321┊325┊            query: USER_QUERY,
 ┊322┊326┊            variables: {
-┊323┊   ┊              id: 1, // faking the user for now
+┊   ┊327┊              id: ownProps.auth.id,
 ┊324┊328┊            },
 ┊325┊329┊          });
 ┊326┊330┊
```
```diff
@@ -335,7 +339,7 @@
 ┊335┊339┊            store.writeQuery({
 ┊336┊340┊              query: USER_QUERY,
 ┊337┊341┊              variables: {
-┊338┊   ┊                id: 1, // faking the user for now
+┊   ┊342┊                id: ownProps.auth.id,
 ┊339┊343┊              },
 ┊340┊344┊              data: userData,
 ┊341┊345┊            });
```
```diff
@@ -346,7 +350,12 @@
 ...
 ┊347┊351┊});
 ┊348┊352┊
+┊   ┊353┊const mapStateToProps = ({ auth }) => ({
+┊   ┊354┊  auth,
+┊   ┊355┊});
+┊   ┊356┊
 ┊349┊357┊export default compose(
+┊   ┊358┊  connect(mapStateToProps),
 ┊350┊359┊  groupQuery,
 ┊351┊360┊  createMessageMutation,
 ┊352┊361┊)(Messages);
```
